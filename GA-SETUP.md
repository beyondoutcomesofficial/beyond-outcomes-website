...
The snippet uses `G-XXXXXXXXXX` as a placeholder. You need to replace this
...
1. In VS Code, press: `Cmd + Shift + H` (or `Cmd + Shift + F` then click the
   replace toggle)
2. In the "Search" box, type: `G-XXXXXXXXXX`
...
Each page has TWO instances of `G-XXXXXXXXXX` to replace:
